...
// 减小到2000以节省内存 (从5000改为2000，节省约84KB)
...
{
    if (g_open_list_size == 0) return NULL;
...
static void reconstruct_path(astar_node_t *goal_node, path_t *path)
{
    ...
    ESP_LOGI(TAG, "Path reconstructed: %d waypoints, cost=%.1f", path->length, path->total_cost);
}
...
esp_err_t astar_init(void)
{
    ...
    g_visited = (uint8_t (*)[MAP_WIDTH])heap_caps_malloc(visited_size, MALLOC_CAP_8BIT);
    if (g_visited == NULL) {
        ESP_LOGE(TAG, "Failed to allocate visited array (%u bytes)", visited_size);
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "  Allocated visited array: %u bytes at %p", visited_size, g_visited);

    // 2. 动态分配开放列表 (约8KB - 中等)
    size_t open_list_size = NODE_POOL_SIZE * sizeof(astar_node_t*);
    g_open_list = (astar_node_t**)heap_caps_malloc(open_list_size, MALLOC_CAP_8BIT);
    if (g_open_list == NULL) {
    ...
    ESP_LOGI(TAG, "  Allocated open list: %u bytes at %p", open_list_size, g_open_list);

    // 3. 动态分配节点池 (约48KB - 最大的，最后分配)
    size_t node_pool_size = NODE_POOL_SIZE * sizeof(astar_node_t);
    g_node_pool = (astar_node_t*)heap_caps_malloc(node_pool_size, MALLOC_CAP_8BIT);
    if (g_node_pool == NULL) {
    ...
    ESP_LOGI(TAG, "  Allocated node pool: %u bytes at %p", node_pool_size, g_node_pool);

    size_t total_size = node_pool_size + open_list_size + visited_size;
    g_astar_initialized = true;
    ESP_LOGI(TAG, "A* initialized successfully (total: %u bytes, ~%u KB)", total_size, total_size / 1024);
    return ESP_OK;
}
                          ...
                          path_t *path)
{
    ...
    ESP_LOGI(TAG, "Planning path from (%d,%d) to (%d,%d)", start_x, start_y, goal_x, goal_y);
    ...
    if (start_node == NULL) return ESP_FAIL;
    ...
    while (g_open_list_size > 0 && iterations < MAX_ASTAR_ITERATIONS) {
        ...
        if (iterations - last_log_iteration >= 1000) {
            ESP_LOGI(TAG, "A* iteration %lu, open_list_size=%d, nodes_used=%d",
                     iterations, g_open_list_size, g_node_pool_index);
        ...
        g_visited[current->y][current->x] = 1;

        // 检查是否到达终点
        if (current->x == goal_x && current->y == goal_y) {
            ESP_LOGI(TAG, "Path found! Iterations: %lu", iterations);
