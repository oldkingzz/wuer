...
esp_err_t tof_init(void) {
  ...
  // 直连模式（不使用TCA9548A）
  Serial.println(
      "Initializing ToF sensors (direct connection, no TCA9548A)...");
  Serial.flush();
#endif
  ...
  if (lox_front.begin()) {
    ...
    // 使用默认测距时间33ms（更稳定）
    // 如果需要更快，可以设置为20ms:
    // setMeasurementTimingBudgetMicroSeconds(20000)
...
esp_err_t tof_read(tof_position_t position, tof_data_t *data) {
  ...
  for (uint8_t retry = 0; retry < max_retries; retry++) {
    ...
    if (retry < max_retries - 1) {
      ESP_LOGW(TAG,
               "ToF channel %d read failed (status=%d, dist=%d), retry %d/%d",
               channel, measure.RangeStatus, measure.RangeMilliMeter, retry + 1,
               max_retries);
      delay(10); // 等待10ms后重试
    }
  }
  ...
  if (!success) {
    ESP_LOGW(TAG, "ToF channel %d failed after %d retries", channel,
             max_retries);
  }
...
esp_err_t tof_read_all(tof_data_t *top_data, tof_data_t *front_data,
                       tof_data_t *left_front_data,
                       tof_data_t *left_rear_data) {
 ...
 * Other tasks can call tof_get_cached_*() to get latest readings without
 * blocking.
 */
  ...
  while (tof_async_running) {
    ...
    if (tof_front_initialized) {
      ...
      if (tof_read(TOF_FRONT, &data) == ESP_OK) {
        ...
        if (xSemaphoreTake(tof_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
          g_front_data = data;
          xSemaphoreGive(tof_mutex);
        }
      }
...
esp_err_t tof_start_async_reading(void) {
  ...
  BaseType_t ret =
      xTaskCreatePinnedToCore(tof_async_reading_task, "tof_async", 4096, NULL,
