...
#define ENCODER2_REVERSED   true   // 左轮编码器（原本就反向）
...
static void control_task(void *param) {
    ...
    while (1) {
        if (g_target.is_moving) {
            ...
            // 3. PID计算PWM
            float left_pwm = pid_v2_compute(&g_pid_left, left_target, left_actual, dt);
            float right_pwm = pid_v2_compute(&g_pid_right, right_target, right_actual, dt);
            ...
            if (++debug_count >= 20) {
                ESP_LOGI(TAG, "[LEFT] target=%.1f actual=%.1f pwm=%.0f | [RIGHT] target=%.1f actual=%.1f pwm=%.0f",
                         left_target, left_actual, left_pwm,
                         right_target, right_actual, right_pwm);
                debug_count = 0;
            }
        } else {
            // 停止所有电机
...
esp_err_t chassis_v2_init(void) {
    ...
    // 1. 初始化PID控制器（左右轮分别调整）
    pid_v2_init(&g_pid_left, PID_LEFT_KP, PID_LEFT_KI, PID_LEFT_KD, PID_OUTPUT_MIN, PID_OUTPUT_MAX);
    pid_v2_init(&g_pid_right, PID_RIGHT_KP, PID_RIGHT_KI, PID_RIGHT_KD, PID_OUTPUT_MIN, PID_OUTPUT_MAX);
    ESP_LOGI(TAG, "PID initialized:");
    ESP_LOGI(TAG, "  Left:  Kp=%.1f Ki=%.1f Kd=%.1f", PID_LEFT_KP, PID_LEFT_KI, PID_LEFT_KD);
    ESP_LOGI(TAG, "  Right: Kp=%.1f Ki=%.1f Kd=%.1f (adjusted for motor issue)", PID_RIGHT_KP, PID_RIGHT_KI, PID_RIGHT_KD);
    ...
    ESP_LOGI(TAG, "  Motor1 (Right) reversed: %s", MOTOR1_REVERSED ? "YES" : "NO");
    ESP_LOGI(TAG, "  Motor2 (Left) reversed: %s", MOTOR2_REVERSED ? "YES" : "NO");
    ESP_LOGI(TAG, "  Encoder1 (Right) reversed: %s", ENCODER1_REVERSED ? "YES" : "NO");
    ESP_LOGI(TAG, "  Encoder2 (Left) reversed: %s", ENCODER2_REVERSED ? "YES" : "NO");
...
esp_err_t chassis_v2_set_velocity(float linear, float angular) {
    ...
    if ((now - last_log) >= pdMS_TO_TICKS(1000)) {
        ESP_LOGI(TAG, "Set velocity: v=%.3f ω=%.3f → L=%.1f R=%.1f RPM",
...
esp_err_t chassis_v2_stop(void) {
    return chassis_v2_set_velocity(0.0f, 0.0f);
}
